...
class ModernMapGenerator:
    ...
    def generate_modern_static_map(self) -> str:
        ...
        # Plotar dados com estilo moderno
        states_web.plot(ax=ax_main, color=self.color_palette['background'], 
                       edgecolor=self.color_palette['border'], alpha=0.4, linewidth=0.8)
        ...
        if containing_state is not None:
            ...
            state_web.plot(ax=ax_state, color=self.color_palette['background'], 
                          edgecolor=self.color_palette['border'], alpha=0.5, linewidth=1.2)
        ...
        if containing_state is not None:
            state_web.plot(ax=ax_country, color=self.color_palette['text_secondary'], 
                          edgecolor=self.color_palette['border'], alpha=0.6, linewidth=1)
    ...
    def _create_modern_info_panel(self, ax_info, gdf_web=None):
        ...
        # Nome da propriedade/imóvel na legenda
        property_name = f"{self.project.name}"
        if len(property_name) > 20:
            property_name = property_name[:17] + "..."
        
        ax_info.text(0.28, legend_y, property_name, 
        ...
        info_lines = [
            ...
            f'• Imóvel: {self.project.name[:20]}...' if len(self.project.name) > 20 else f'• Imóvel: {self.project.name}'
        ]
    ...
    def _add_modern_north_arrow(self, fig_main, ax_main):
        ...
        try:
            ...
            if os.path.exists(north_arrow_path):
                ...
                # Posicionar com sombra sutil
                newax = fig_main.add_axes([0.05, 0.75, 0.18, 0.18], anchor='NW', zorder=3)
